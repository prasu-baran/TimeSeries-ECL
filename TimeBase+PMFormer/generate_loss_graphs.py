...
def draw_panel(draw, rect, title, x_label, y_label, points, color, value_suffix=""):
    x0, y0, x1, y1 = rect
    left = x0 + 72
    right = x1 - 22
    top = y0 + 48
...
def generate(results_dir):
    ...
    for model in ["TimeBase", "TimeBase+PMFormer"]:
        for horizon in [1, 8, 16, 24]:
            ...
            prefix = "timebase_pmformer" if model == "TimeBase+PMFormer" else "timebase"
